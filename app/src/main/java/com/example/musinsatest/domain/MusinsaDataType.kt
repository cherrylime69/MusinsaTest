...
sealed class MusinsaDataType(val dataType: Int) {
    ...
    data class ContentType(val type: Int = ViewType.DATA_TYPE_CONTENT, val data: MusinsaContentType) : MusinsaDataType(type)
...
fun MusinsaData.divideDataType(): List<MusinsaDataType> {
    return listOf(
        ...
        MusinsaDataType.ContentType(data = this.contents.divideContentType()),
